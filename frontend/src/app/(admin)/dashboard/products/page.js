...
import { Home, Plus, Edit, Trash, Download, Eye, ListFilter, LayoutList } from 'lucide-react';
...
const Product = () => {
  ...
  const handleAddProduct = async () => {
    try {
      ...
      await axios.post(`${apiUrl}/api/addproduct`, submitData, { headers: { 'Content-Type': 'multipart/form-data' } });

      Swal.fire({ icon: 'success', title: 'Product Added!', timer: 1500, showConfirmButton: false });
      setShowAddModal(false);
      fetchData();
    } catch (err) {
      Swal.fire({ icon: 'error', title: 'Failed to add product', text: err.message });
    }
  };
  ...
  const handleToggleStatus = async (prod) => {
    try {
      ...
      await axios.put(`${apiUrl}/api/product/status/${prod.prod_id}`, { status: newStatus });
      fetchData();
    } catch (err) {
      console.error(err);
    }
...
const columns = [
...

  {
    ...
    renderCell: (params) => (
      <button
        className={`btn btn-sm ${params.value === 1 ? 'btn-success' : 'btn-danger'}`}
  ...
  },
  {
    ...
    renderCell: (params) => (
      <Box sx={{ display: 'flex', gap: 1 }}>
        ...
        <IconButton color="error" onClick={() => handleDelete(params.row.prod_id)}>
          <Trash size={18} />
        </IconButton>
      </Box>
    ),
 ...
 <div style={{ width: '100%', overflowX: 'auto' }}>
  <div style={{ minWidth: 1800 }}> {/* sum of all column widths */}
    <DataGrid
      rows={rows}
      columns={columns}
      pageSize={10}
              ...
              <div className="modal-body">
                <div className="row g-3">
                  <div className="col-md-6">
                    ...
                    <input type="text" className="form-control" name="prod_name" value={formData.prod_name} onChange={handleChange}/>
                  </div>
                  <div className="col-md-6">
                    <label className="form-label">Price</label>
                    <input type="number" className="form-control" name="prod_price" value={formData.prod_price} onChange={handleChange}/>
                  </div>
                  <div className="col-md-6">
...
>
  ...
  {subcategories
    .filter(sc => sc.cat_id === Number(formData.category_id)) // convert to number if needed
    .map(sc => (
  ...
  >
    ...
    {items
      .filter(item => item.sub_id === Number(formData.subcategory_id)) // filter by selected subcategory
      .map(item => (
        <option key={item.item_id} value={item.item_id}>
          {item.item_name}
                  ...
                  <div className="col-md-6">
                    ...
                    <select className="form-select" name="badge_id" value={formData.badge_id} onChange={handleChange}>
                      <option value="">Select Badge</option>
                      {badges.map(b => <option key={b.badge_id} value={b.badge_id}>{b.badge_name}</option>)}
                    </select>
                  </div>
                  <div className="col-md-6">
                    <label className="form-label">Status</label>
                    <select className="form-select" name="prod_status" value={formData.prod_status} onChange={handleChange}>
            ...
            <div className="col-md-6">
              ...
              />
                  ...
                  <div className="col-md-12">
                    ...
                    <textarea className="form-control" name="prod_des" value={formData.prod_des} onChange={handleChange}></textarea>
                  </div>
                  <div className="col-md-6">
                    <label className="form-label">Review</label>
                    <input type="number" step="0.1" min="0" max="5" name="prod_review" className="form-control" value={formData.prod_review} onChange={handleChange}/>
                  </div>
                  <div className="col-md-6">
                  ...
                  <div className="col-md-6">
                    ...
                    <input type="text" name="prod_color" className="form-control" value={formData.prod_color} onChange={handleChange}/>
                  </div>
                  <div className="col-md-6">
                    <label className="form-label">Dimensions</label>
                    <input type="text" name="prod_dimensions" className="form-control" value={formData.prod_dimensions} onChange={handleChange}/>
                  </div>
                  <div className="col-md-12">
                    <label className="form-label">Product Think</label>
                    <textarea className="form-control" name="prod_think" value={formData.prod_think} onChange={handleChange}></textarea>
                  </div>
                  <div className="col-md-12">
                    <label className="form-label">Images</label>
                    <input type="file" multiple onChange={handleImageChange} className="form-control"/>
                    <div className="mt-2 d-flex flex-wrap gap-2">
                      {imagePreviews.map((src, idx) => <img key={idx} src={src} width={80} height={80} className="rounded border"/> )}
                    </div>
                  </div>
                </div>
              </div>
...
>
  <option value="">Select Subcategory</option>
  {subcategories
    .filter(sc => sc.cat_id === Number(editFormData.category_id))
    .map(sc => (
      <option key={sc.sub_id} value={sc.sub_id}>
        {sc.sub_name}
      </option>
            ...
            {/* Product Think */}
            <div className="col-md-12">
              <label className="form-label">Product Think (Features / Additional Info)</label>
              <textarea
                className="form-control"
                name="prod_think"
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-warning text-white" onClick={handleUpdateProduct}>Update</button>
        </div>
      </div>
    </div>
  </div>
